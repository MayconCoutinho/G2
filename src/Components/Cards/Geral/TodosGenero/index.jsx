...
export const TodosGenero = ({ newsletter, pageNow }) => {
  ...
  const TodosGeneroNewsletter = JSON.parse(JSON.stringify(newsletter));
  TodosGeneroNewsletter.length = vejaMais
  useEffect(() => {
    setVejaMais(3)
  }, [pageNow])
  ...
  return <> {
    ...
    pageNow.toLowerCase() === "geral" ?

      TodosGeneroNewsletter.map((item, index) => {
        return <>
          <Grid
            container
            direction={"row"}
